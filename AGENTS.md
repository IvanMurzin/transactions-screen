...
> A copy of this guide for Claude Code lives in `CLAUDE.md`. Both files
> must stay in sync — see the `check-ai-consistency` skill.
...
1. **Explore.** Read the relevant spec, the product docs it links, and
   the code paths it touches. Cast a wide net before narrowing in.
...
Located in `.agents/skills/<name>/SKILL.md`. Listed by trigger:
...
Codex doesn't ship a native subagent execution model the way Claude
Code does. Treat these files as prompt snippets you load into context
when adopting that role for a focused review or research task.
